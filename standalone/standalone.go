...
func Handler(ch grpcdynamic.Channel, target string, methods []*desc.MethodDescriptor, files []*desc.FileDescriptor, opts ...HandlerOption) http.Handler {
	...
	indexContents := getIndexContents(uiOpts.indexTmpl, target, uiOpts.title, webFormHTML, uiOpts.tmplResources, uiOpts.catalogURL)
...
func getIndexContents(tmpl *template.Template, target, title string, webFormHTML []byte, addlResources []*resource, catalogURL string) []byte {
